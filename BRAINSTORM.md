...
- **Packaging: ✔ v1.0 SHIPPED 2026-08-31** — Electron shell (`electron-main.mjs` runs the server in-process, window at 127.0.0.1:5892) + electron-builder portable target → `dist\RallesAQ2TextureSwapper.exe` (~98 MB, one file, logo icon). First-run: friendly welcome/Browse prompt when no install found, orange setup banner for the hook. Verified end-to-end (boot, embedded server, 1804-map scan). Deferred to v1.1: viewer skybox, sound swaps, team packs.
