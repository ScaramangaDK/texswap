...
- **V1.1 startup polish** (user request 2026-08-31): loading/splash screen so the app never looks frozen on launch. Two waits to cover: (a) the portable exe self-extracting before any window shows (~2-5s) - show the BrowserWindow immediately with a branded "loading" page, or switch off the portable target; (b) the first map scan (~10-15s on a 1800-map install) while the map list sits empty - progress UI in the sidebar ("scanning maps... 400/1804"), which needs the scan endpoint to stream progress or the UI to poll a /api/scanstatus.
